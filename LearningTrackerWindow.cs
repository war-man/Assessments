...

    {
        ...
        */
        ...
        private void btnShowChaptObj_Click(object sender, EventArgs e)
        {
            ...
            //if user is choosing another chapter's list, we will make the 4th group disappear until they select an objective and generate groups again (not to confuse them thinking the all stars have been generated by clicking the show chapter button).
            pnl4thGroup.Visible = false;
        ...
        private void btnGoGroups_Click(object sender, EventArgs e)
        {
            ...
            if (scRosterDisplay.Visible == true)
            {
                ...
                if (objecCount > 0)
                {
                    ...
                    //only display 4thGroup box if a single chapter had been selected from the lvChapters box.
                    if (sto.CurChapObjectives != null && sto.CurrentChapters != null)
                    {
                    ...
                    else
                    {
                        ...
                        if (objCount == 1)
                        {
                            lblFeedbackMessage.Text = "If you want to generate 'All-Star' groups, you must first select and show objectives for only ONE chapter";
                            lblFeedbackMessage.Visible = true;
                        }
        ...
        /// <summary>
        /// Determines if student object has completed ALL objectives in the current chapter that is selected from lvChapters
        /// </summary>
        /// <param name="possibleAllStar">Student to evaluate</param>
        /// <returns>true if student has completed all objectives in selected chapter successfully</returns>
        private bool AllObjectivesComplete(Student possibleAllStar)
        {
            //if no specific chapter has been selected then there is no way to determine if student has completed all the chatper's objectives
            if ((sto.CurChapObjectives == null) || (!(sto.CurChapObjectives.Count > 0)))
            {
                return false;
            }

            // assumes completion of all chapter objectives until found to be otherwise
            bool ARE_THEY = true;
            ...
            foreach (string s in sto.CurChapObjectives)
            {
                if (ARE_THEY == true)
                {
                    foreach (Objective o in possibleAllStar.Completed)
                    {
                        ...
                        {
                            ARE_THEY = true;
                            //if already determined student completed this objective then we need to stop iterating
                            break;
                        }
                        else
                        {
        ...
        private void DetermineCurChaptObjs()
        {
            ...
            {
                // if currentchapters is null then we know that there was more than one chapter being displayed in the OBJ box OR more than one chapter's objective selected in the objective box
                sto.CurrentChapters = null;
            }
        }
        ...
        private StringBuilder CopyAllGroupBoxesString()
        {
            ...
            //will hold the full strings element to copy to clipboard.
            StringBuilder fullSb = new StringBuilder();
            
            //will hold the list of strings from each box
            string temp;
            string barrier = "--------------------";

            //generates strings for items in "All Star" box
            ...
            if (sto.CurrentChapters != null)
            {
                ...
                if (temp == null)
                {
                    ...
                    //skips a line between next group
                    fullSb.AppendLine();
                }
                else
                {
                    //places temp strings under the All-Stars for Chapter ...
                    fullSb.AppendLine(temp);
                }
            }

            //sto.CurrentChapters.Count is not equal to one
            else
            {
                fullSb.AppendLine("~~~~All-Star data not available for this type of Group request~~~~");
            ...
            //generates strings for items in "Completed Box"
            CopyListViewBox(lvCompleted, out temp);
            
            fullSb.AppendLine("**COMPLETE**");

            //nothing was in the "Completed" box
            if (temp == null)
            {
                fullSb.AppendLine("//NONE//");
            ...
            {
                //places temp strings under the heading **COMPLETE**
                fullSb.AppendLine(temp);
            }


            /*generates strings for items in "Partially Completed" box: overwrites (or just changes pointer) the temp string used previously*/
            CopyListViewBox(lvPartial, out temp);

            fullSb.AppendLine("**PARTIALLY**");

            //nothing was in the partially completed box
            if (temp == null)
            {
                //tells user that there was no one in this group
                fullSb.AppendLine("//NONE//");
            ...
            {
                // places temp strings under the heading* Partially*
                 fullSb.AppendLine(temp);
            }

            //generates strings for items in "Incomplete" box
            CopyListViewBox(lvNot, out temp);
            fullSb.AppendLine("**INCOMPLETE**");

            //if no items were in the incomplete box
            if (temp == null)
            {
            ...
            {
                //places temp strings under the heading "Incomplete"
                fullSb.AppendLine(temp);
            }
            return fullSb;
        }
